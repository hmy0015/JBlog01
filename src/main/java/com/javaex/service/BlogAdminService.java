...
public class BlogAdminService {
	...
	public BlogVo getBlogInfo(String id) {
		System.out.println("service - 현재 블로그 정보 가져오기");
	...
	public int modifyBasic(String id, BlogVo blogVo, MultipartFile file) {
		System.out.println("service - 기본 설정 수정 ");
		...
		if(file.isEmpty()) {
			...
			// 하드에 저장 될 파일의 이름
			saveName = System.currentTimeMillis() + UUID.randomUUID().toString() + exName; // 현재 시간 + 영문과 숫자를 조합한 랜덤 이름 + 확장자
		...
		bVo = new BlogVo(id, blogVo.getBlogTitle(), saveName);
	...
	public List<CategoryVo> getCategoryList(String id) {
		System.out.println("service - 카테고리 정보 불러오기 ");
	...
	public CategoryVo addCate(String id, CategoryVo cateVo) {
		System.out.println("service - 카테고리 추가하기 ");
	...
	public int delCate(int cateNo) {
		System.out.println("service - 카테고리 삭제하기 ");
	...
	public int write(PostVo postVo) {
		System.out.println("service - 게시글 등록 ");
